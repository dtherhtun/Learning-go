...
func newSummary(ctx context.Context, config *pomodoro.IntervalConfig, redrawCh chan<- bool, errorCh chan<- error) (*summary, error) {
	...
	s.bcDay, err = newBarChat(ctx, config, s.updateDaily, errorCh)
	if err != nil {
		return nil, err
	}
...
func newBarChat(ctx context.Context, config *pomodoro.IntervalConfig, update <-chan bool, errorCh chan<- error) (*barchart.BarChart, error) {
